  ...
  "A json string" when {
    ...
    """{"poorSignalLevel":199}""" should {
      ...
      "be parsed in PoorSignalLevel(199)" in {
        ParserLiftJson.parse(json) shouldBe ThinkgearResponse(poorSignalLevel=Some(199))
      }
    }
    ...
    """{"blinkStrength":41}""" should {
      ...
      "be parsed in Blink(41)" in {
        ParserLiftJson.parse(json) shouldBe ThinkgearResponse(blinkStrength=Some(41))
     }
    }
    ...
    """{"rawEeg":-123}""" should {
      ...
      "be parsed in RawData(-123)" in {
        ParserLiftJson.parse(json) shouldBe ThinkgearResponse(rawEeg=Some(-123)) 
      }
    }
    ...
    """{"eSense":{"attention":75,"meditation":38},"eegPower":{"delta":247328,"theta":98222,"lowAlpha":27763,"highAlpha":6216,"lowBeta":9065,"highBeta":12236,"lowGamma":7982,"highGamma":5343},"poorSignalLevel":0}""" should {
      ...
      "be parsed in " in {
        ParserLiftJson.parse(json) shouldBe ThinkgearResponse(
